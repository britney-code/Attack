...
def get_parser():
    ...
    parser.add_argument('--output_dir', default='./result/mifgsm', type=str, help='the path to store the adversarial patches')
...
def main():
    ...
    dataset = AdvImagetNet(input_dir=args.input_dir, output_dir=args.output_dir, targeted=args.targeted, eval=args.eval)
    ...
    if not args.eval:
        ...
        end_time = time.time()
        print(f"{args.attack} execution time: {end_time - start_time:.2f}s") 
    else:
        res = '|'
        model_count = 0 
        total_count = 0 
        for model_name, model in load_pretrained_model(cnn_model_paper, vit_model_paper, adv_model_paper): # vit_model_paper  adv_model_paper
            ...
            if args.attack in ['ttp', 'm3d']: 
                ...
                asr = eval(model, dataloader, args.targeted, model_name)
            model_count += 1 
            total_count += asr 
            print(f'{model_name}: {asr:.1f}')
            res += f' {asr:.1f} |'
        AVG = total_count / model_count if model_count > 0 else 0 
        res += f' {AVG:.1f} |'
        print(f"average AVG:{AVG:.1f}")
        print(res)
        with open('results_eval.txt', 'a') as f:
            f.write(args.output_dir + res + '\n')
                
                
def eval(model, dataloader, is_targeted, model_name):
        correct, total = 0, 0
        for images, labels, _ in dataloader:
            ...
            if 'adv' not in model_name:
                ...
                correct += (labels.numpy() + 1 == pred.argmax(dim=1).detach().cpu().numpy()).sum()
